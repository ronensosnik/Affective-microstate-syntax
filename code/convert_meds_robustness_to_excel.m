...
if nargin < 2 || isempty(outFile)
    outFile = fullfile(rootDir, 'Metrics_Meds_Robustness.xlsx');
end
...
try
    readme = {'GeneratedBy', 'convert_meds_robustness_to_excel.m' 'GeneratedOn', datestr(now, 'yyyy-mm-dd HH:MM:SS') 'SourceDir', char(rootDir) 'Notes', 'Each measure has "classes" (per-class) and "poly" (poly/pooled) specs, each with detail and summary sheets.'};
    ...
    try
        ...
        if isscalar(x) && all(cellfun(@(fn) istable(x.(fn)), f))
            ...
            for i = 2: numel(f)
                ...
                Ti.Properties.VariableNames = matlab.lang.makeUniqueStrings(strcat(f{i}, "_", string(Ti.Properties.VariableNames)), [T.Properties.VariableNames]);
                T = [T Ti];
            end
            return;
        end
