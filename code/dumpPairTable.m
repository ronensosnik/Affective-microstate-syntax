...
elseif strcmpi(factor,'Condition') && all(ismember({'Condition_1','Condition_2'}, T.Properties.VariableNames))
    L1 = string(T.Condition_1); L2 = string(T.Condition_2);
    if ~isempty(ctxVar) && ismember(ctxVar, T.Properties.VariableNames)
        ctx = string(T.(ctxVar));
    else
        ctx = repmat("", height(T), 1);
    ...
    if ~isempty(lefts) && ~isempty(rights)
        L1 = string(T.(lefts{1})); L2 = string(T.(rights{1}));
    else
        L1 = repmat("", height(T), 1); L2 = L1;
    end
    ctx = repmat("", height(T), 1);
end
...
C = cell(height(T), 15);
for i = 1:height(T)
    C(i,:) = {char(measure), char(ERP), char(Micro), char(scope), char(factor), ...
        char(ctx(i)), char(L1(i)), char(L2(i)), ...
        eff(i), lo(i), hi(i), T.pValue(i), T.pFDR(i), char(units), char(pipeline_label)};
end
end
