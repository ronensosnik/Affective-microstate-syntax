...
for e = 1:3
    for m = 1:7
        ...
        if isfield(R.pairwise,'Group') && ~isempty(R.pairwise.Group)
            rows = [rows; dumpPairTable(R.pairwise.Group,     measure, ERPs{e}, Micros{m}, 'Group (marginal)',     'Group',     '',          pipeline_label)]; %#ok<AGROW>
        end
        if isfield(R.pairwise,'Condition') && ~isempty(R.pairwise.Condition)
            rows = [rows; dumpPairTable(R.pairwise.Condition, measure, ERPs{e}, Micros{m}, 'Condition (marginal)', 'Condition', '',          pipeline_label)]; %#ok<AGROW>
        end
        ...
        if isfield(R.pairwise,'Group_within_Condition') && ~isempty(R.pairwise.Group_within_Condition)
            rows = [rows; dumpPairTable(R.pairwise.Group_within_Condition, ...
                measure, ERPs{e}, Micros{m}, 'Group @ Condition', 'Group', 'Condition', pipeline_label)]; %#ok<AGROW>
        end
        if isfield(R.pairwise,'Condition_within_Group') && ~isempty(R.pairwise.Condition_within_Group)
            rows = [rows; dumpPairTable(R.pairwise.Condition_within_Group, ...
                measure, ERPs{e}, Micros{m}, 'Condition @ Group', 'Condition', 'Group', pipeline_label)]; %#ok<AGROW>
        end
    end
end
...
if isempty(rows)
    ...
    T = cell2table(cell(0,15), 'VariableNames', ...
        {'Measure','ERP','Microstate','Scope','Factor','Context','Level1','Level2','Effect','CI_low','CI_high','p','pFDR','Units','Pipeline'});
