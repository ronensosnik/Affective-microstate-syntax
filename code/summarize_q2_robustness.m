...
total_base = 0; retain_dir = 0; retain_sig = 0;
per_erp = struct();
...
for e = 1:numel(ERPs)
    ...
    for c = 1:numel(Conds)
        cond = Conds{c};
        if ~isfield(Base.(erp),cond) || ~isfield(Med.(erp),cond), continue; end
        ...
        for k = 1:numel(tgs)
            tgt = tgs{k};
            B = Base.(erp).(cond).(tgt);
            if ~isfield(Med.(erp).(cond), tgt), continue; end
            M = Med.(erp).(cond).(tgt);

            sigB = B.sig; if isempty(sigB), continue; end
            for s = 1:7
                for t = 1:7
                    if s==t, continue; end
                    if sigB(s,t)
                        total_base = total_base + 1; bE = bE + 1;
    ...
    per_erp.(erp) = struct('BaselineEdges', bE, ...
...
overall = struct('BaselineEdges', total_base, ...
    'DirectionRetained', retain_dir, ...
    'SignificanceRetained', retain_sig, ...
    'DirPct', pct(retain_dir,total_base), ...
    'SigPct', pct(retain_sig,total_base));

% Make a by-ERP table
byERP = table('Size',[numel(ERPs) 5], ...
