    ...
    for c = 1: numel(Conds)
        ...
        if ~isempty(RefCell)
            ...
            for sj = 1: numel(RefCell)
                ...
                switch opt.Spec
                    case 'classes'
                        R = table(yvec(keep), logE, repmat(agesRef(min(sj, end)), nnz(keep),1), repmat(medsRef.AD(min(sj, end)), nnz(keep), 1), repmat(medsRef.AP(min(sj, end)), nnz(keep), 1), repmat(medsRef.MS(min(sj, end)), nnz(keep), 1), repmat(medsRef.ANX(min(sj, end)), nnz(keep), 1), repmat(medsRef.OTHER(min(sj, end)), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeRaw', 'AD', 'AP', 'MS', 'ANX', 'OTHER', 'src', 'tgt'});
                    case 'poly'
                        R = table(yvec(keep), logE, repmat(agesRef(min(sj, end)), nnz(keep), 1), repmat(medsRef.poly_count(min(sj, end)), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeRaw', 'poly_count', 'src', 'tgt'});
                end
                Tref = [Tref; R];
            end
        end
        ...
        for g = 1: numel(TargetList)
            ...
            if ~isempty(TgtCell)
                ...
                for sj = 1: numel(TgtCell)
                    ...
                    switch opt.Spec
                        case 'classes'
                            R = table(yvec(keep), logE, repmat(agesT(min(sj, end)), nnz(keep), 1), repmat(medsT.AD(min(sj, end)), nnz(keep), 1), repmat(medsT.AP(min(sj, end)), nnz(keep), 1), repmat(medsT.MS(min(sj, end)), nnz(keep), 1), repmat(medsT.ANX(min(sj, end)), nnz(keep), 1), repmat(medsT.OTHER(min(sj,end)), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeRaw', 'AD', 'AP', 'MS', 'ANX', 'OTHER', 'src', 'tgt'});
                        case 'poly'
                            R = table(yvec(keep), logE, repmat(agesT(min(sj, end)), nnz(keep), 1), repmat(medsT.poly_count(min(sj, end)), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeRaw', 'poly_count', 'src', 'tgt'});
                    end
                    Ttgt = [Ttgt; R];
                end
            end
            ...
            muR = mean(Tref.AgeRaw, 'omitnan');
            sdR = std(Tref.AgeRaw, 'omitnan');
            Rref = Tref;

            if sdR <= eps
                Rref.AgeS = zeros(height(Rref), 1);
            else
                Rref.AgeS = (Rref.AgeRaw - muR)./sdR;
            end
            ...
            for k = 1: numel(srcIdx)
                ...
                if sum(Rref_k.y) < opt.MinObs || sum(exp(Rref_k.logE)) < opt.MinExp
                    continue;
                end

                try
                    mdlRef = fitglm(Rref_k, 'y ~ 1 + AgeS', 'Distribution', 'poisson', 'Link', 'log', 'Offset', Rref_k.logE, 'Options', GLM_OPTS);
                ...
                if isempty(R2_ref) || isempty(R2_tgt)
                    continue;
                end
                ...
                if sum(R2.y(R2.Gnum == 0)) == 0 || sum(R2.y(R2.Gnum == 1)) == 0
                    continue;
                end
                ...
                try
                    switch opt.Spec
                        case 'classes'
                            mdl2 = fitglm(R2, 'y ~ -1 + Gnum + AD + AP + MS + ANX + OTHER', 'Distribution', 'poisson', 'Link', 'log', 'Offset', R2.offset_ref, 'Options', GLM_OPTS);
                        case 'poly'
                            mdl2 = fitglm(R2, 'y ~ -1 + Gnum + poly_count', 'Distribution', 'poisson', 'Link', 'log', 'Offset', R2.offset_ref, 'Options', GLM_OPTS);
                    end
            ...
            if ~isempty(sSig)
                ...
                edges = table(sSig, tSig, rr, lo, hi, qv, 'VariableNames', {'src', 'tgt', 'IRR', 'IRR_CIlo', 'IRR_CIhi', 'qFDR'});
