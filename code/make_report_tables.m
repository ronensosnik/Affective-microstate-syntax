...
for e = 1: numel(ERPs)
    for m = 1: numel(Micros)
        ...
        omniRow = table(string(measure), string(ERPs{e}), string(Micros{m}), string(R.model_used), R.tests.interaction.p, R.tests.interaction.pFDR, R.tests.group.p, R.tests.group.pFDR, R.tests.condition.p, R.tests.condition.pFDR, 'VariableNames', {'Measure', 'ERP', 'Microstate', 'ModelUsed', 'p_Interaction', 'pFDR_Interaction', 'p_Group', 'pFDR_Group', 'p_Condition', 'pFDR_Condition'});
        Omni = [Omni; omniRow];
        ...
        if any(strcmp(A.Term, 'Age_c'))
            ...
            ageRow = table(string(measure), string(ERPs{e}), string(Micros{m}), rowA.DF1, rowA.DF2, rowA.FStat, rowA.pValue, 'VariableNames', {'Measure', 'ERP', 'Microstate', 'DF1', 'DF2', 'F_Age', 'p_Age'});
            AgeT = [AgeT; ageRow];
        else
            ageRow = table(string(measure), string(ERPs{e}), string(Micros{m}), NaN, NaN, NaN, NaN, 'VariableNames', {'Measure', 'ERP', 'Microstate', 'DF1', 'DF2', 'F_Age', 'p_Age'});
            AgeT = [AgeT; ageRow];
        end
        ...
        if isfield(R, 'emm')
            ...
            for k = 1: numel(fns)
                ...
                if ~isempty(T)
                    ...
                    keep = intersect({'Measure', 'ERP', 'Microstate', 'Source', 'Group', 'Condition', 'EMM_BT', 'EMM_BT_Low', 'EMM_BT_High', 'Units', 'N_subj'}, T2.Properties.VariableNames, 'stable');
                    emmTables{end + 1} = T2(:, keep);
                end
            end
        end

        if ~isempty(emmTables)
            EMMs = [EMMs; vertcat(emmTables{:})];
        end
