function h = daviolinplot_microstate_task(Y, varargin)

%% daviolinplot_microstate_task.m
...
if ~isempty(confs.groups)
    if strcmp(confs.groups{1}, 'BP_I_Depressed')
       Gi = [ones(1, length(find(strcmp(confs.groups, 'BP_I_Depressed'))))'; 2 * ones(1, length(find(strcmp(confs.groups, 'BP_I_Euthymic'))))'; 3 * ones(1, length(find(strcmp(confs.groups, 'BP_II_Depressed'))))'; 4 * ones(1, length(find(strcmp(confs.groups, 'BP_II_Euthymic'))))'; 5 * ones(1, length(find(strcmp(confs.groups, 'HC'))))'; 6 * ones(1, length(find(strcmp(confs.groups, 'Siblings'))))'];
    Gn{1, 1} = 'BP_I_Depressed';
    Gn{2, 1} = 'BP_I_Euthymic';
    Gn{3, 1} = 'BP_II_Depressed';
    Gn{4, 1} = 'BP_II_Euthymic';
    Gn{5, 1} = 'HC';
    Gn{6, 1} = 'Siblings';

    num_groups = 6;

    elseif strcmp(confs.groups{1}, 'Neutral')
       Gi = [ones(1, length(find(strcmp(confs.groups, 'Neutral'))))'; 2 * ones(1, length(find(strcmp(confs.groups, 'Negative'))))'; 3 * ones(1, length(find(strcmp(confs.groups, 'Positive'))))'];
    Gn{1, 1} = 'Neutral';
    Gn{2, 1} = 'Negative';
    Gn{3, 1} = 'Positive';
...
for g = 1: num_groups
    ...
    end % for plotting one condition
    ...
    for k = 1: num_locs
        ...
        if strcmp(confs.smoothing, 'default')
            [f, xi] = ksdensity(data_vals, 'Kernel', 'normal');
        else
        ...
        if strcmp(confs.violin, 'full')
            h.ds(k,g) = fill([f, -fliplr(f)] + gpos(g, k), [xi, fliplr(xi)], confs. colors(g, :));
        ...
        if confs.box ~= 0
            ...
            if strcmp(confs.boxcolors, 'same')
                h.bx(k, g) = fill([Xx(:, 1)' Xx(1, :) Xx(:, 2)' Xx(2, [2, 1])], [Yy(:, 1)' Yy(1, :) Yy(:, 2)' Yy(2, :)], confs.colors(g, :));
            else
                h.bx(k, g) = fill([Xx(:, 1)' Xx(1, :) Xx(:, 2)' Xx(2, [2, 1])], [Yy(:, 1)' Yy(1, :) Yy(:, 2)' Yy(2, :)], bcol);
            end
            ...
            if confs.whiskers == 1
                ...
                h.wh(k, g, :) = plot(whi_xcor(:, k), whi_ycor(:, 1, k), 'k-', whi_xcor(:, k),whi_ycor(:, 2, k), 'k-', 'LineWidth', 1.5);
            end
        end
        ...
        elseif confs.jitter == 1
            xdata = gpos(g, k).* ones(numel(Y(Gi == g, k)), 1) + (box_width / 1.5).* (0.5 - rand(numel(Y(Gi == g, k)), 1));
        ...
        elseif confs.jitter == 2
            ...
            for i = 1: numel(N)
                for j = 1: N(i)
                    hy = [hy, bm(i)];
                    hx = [hx, (- j + 0.8) * confs.jitterspacing / (8 * numel(gpos)) + gpos(g, k)];
                end
            end
            xdata = hx;
            data_vals = hy;
        ...
        if confs.outliers == 1
            ox = data_vals < (pt(3,k) - confs.outfactor * IQR(k)) | data_vals > (pt(5, k) + confs.outfactor * IQR(k));
        ...
        if confs.scatter ~= 0
            if strcmp(confs.scattercolors, 'same')
                h.sc(k, g) = scatter(xdata(~ox), data_vals(~ox), confs.scattersize, 'MarkerFaceColor', confs.colors(g, :), 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', confs.scatteralpha);
            else
                h.sc(k, g) = scatter(xdata(~ox), data_vals(~ox), confs.scattersize, 'MarkerFaceColor', scol, 'MarkerEdgeColor', ecol, 'MarkerFaceAlpha', confs.scatteralpha);
            end
        end
    end
    ...
    if confs.linkline == 1
        h.ln(g) = line(gpos(g, :), pt(4, :), 'color', confs.colors(g, :), 'LineStyle', '-.', 'LineWidth', 1.5);
    end
    ...
    if confs.withinlines == 1
        for s = 1: size(scdata{g}, 1)
            h.wl(g) = plot(squeeze(scdata{g}(s, 1, :)), squeeze(scdata{g}(s, 2, :)), 'color', [0.8 0.8 0.8]);
