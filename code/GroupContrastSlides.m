        ...
        if isfield(CompStats, erp) && isfield(CompStats.(erp), cond) && isfield(CompStats.(erp).(cond), grp)
            ...
            if can_recompute
                ...
                if ~isempty(sSig)
                    ...
                    dlab(E.IRR > 1) = "Higher vs " + string(refGroup);
                    dlab(E.IRR < 1) = "Lower vs " + string(refGroup);
                    E.Direction = dlab;
                else
            ...
            elseif isfield(node, 'edges')
                ...
                if ~ismember('Direction', E.Properties.VariableNames)
                    if ~isempty(E)
                        ...
                        E.Direction(E.IRR > 1) = "Higher vs " + string(refGroup);
                        E.Direction(E.IRR < 1) = "Lower vs " + string(refGroup);
                    else
                        E = table([], [], [], [], [], [], strings(0, 1), 'VariableNames', {'src', 'tgt', 'IRR', 'IRR_CIlo', 'IRR_CIhi', 'qFDR', 'Direction'});
                    end
                end
