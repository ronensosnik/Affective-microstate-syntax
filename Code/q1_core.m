function TSC = q1_core(Out, AgeMap, ERPs, Groups, Conds, FDR_Q1_SCOPE, GLM_OPTS, LOGE_CLAMP, MIN_OBS, MIN_EXP, MicrostateLabels, spec, alphaFDR, MedMap)
    ...
    for g = 1: numel(Groups)
        ...
        for c = 1: numel(Conds)
            ...
            if ~isempty(cellOut)
                for sj = 1:numel(cellOut)
                    ...
                    switch spec
                        ...
                        case 'classes'
                            R = table(yvec(keep), logE, repmat(AgesZ(sj), nnz(keep), 1), repmat(AD(sj), nnz(keep), 1), repmat(AP(sj), nnz(keep), 1), repmat(MS(sj), nnz(keep), 1), repmat(ANX(sj), nnz(keep), 1), repmat(OTH(sj), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeZ', 'AD', 'AP', 'MS', 'ANX', 'OTHER', 'src', 'tgt'});
                        case 'poly'
                            R = table(yvec(keep), logE, repmat(AgesZ(sj), nnz(keep), 1), repmat(PC(sj), nnz(keep), 1), srcIdx(keep), tgtIdx(keep), 'VariableNames', {'y', 'logE', 'AgeZ', 'poly_count', 'src', 'tgt'} );
                    end
                    RowsAll = [RowsAll; R];
                end
            end
            ...
            for k = 1: numel(srcIdx)
                ...
                try
                    switch spec
                        ...
                        case 'classes'
                            mdl = fitglm(R, 'y ~ 1 + AgeZ + AD + AP + MS + ANX + OTHER', 'Distribution', 'poisson', 'Link', 'log', 'Offset', R.logE, 'Options', GLM_OPTS);
                        case 'poly'
                            mdl = fitglm(R, 'y ~ 1 + AgeZ + poly_count', 'Distribution', 'poisson', 'Link', 'log', 'Offset', R.logE, 'Options', GLM_OPTS);
                    end
            ...
            if ~isempty(sSig)
                ...
                lab(irr_vec > 1) = "Above expected";
                lab(irr_vec < 1) = "Below expected";
                edges.Direction = lab;
            else
