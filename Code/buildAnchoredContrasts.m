    ...
    for c = 1: numel(Conds)
        ...
        if ~isempty(RefCell) && ~isempty(agesRef)
            ...
            for sj = 1: numel(RefCell)
                ...
                R = table(yvec(keep), eKeep, logE, repmat(ageS, nnz(keep), 1), srcIdx(keep), tgtIdx(keep), zeros(nnz(keep), 1), 'VariableNames', {'y', 'e', 'logE', 'AgeS', 'src', 'tgt', 'Target'});
        ...
        for g = 1: numel(TargetList)
            ...
            for sj = 1: numel(TgtCell)
                ...
                R = table(yvec(keep), eKeep, logE, repmat(ageS, nnz(keep), 1), srcIdx(keep), tgtIdx(keep), ones(nnz(keep), 1), 'VariableNames', {'y', 'e', 'logE', 'AgeS', 'src', 'tgt', 'Target'});
                Table_target = [Table_target; R];

                clear aaaa; aaaa = cell2mat(S.transition_duration_matrix(sub2ind([7, 7], srcIdx, tgtIdx)));

                try
                    Dur_src.(erp).(condName).(tgtGrp).data = [Dur_src.(erp).(condName).(tgtGrp).data aaaa(:, 1)];
                    Dur_trg.(erp).(condName).(tgtGrp).data = [Dur_trg.(erp).(condName).(tgtGrp).data aaaa(:, 2)];

                catch
                    Dur_src.(erp).(condName).(tgtGrp).data = aaaa(:, 1);
                    Dur_trg.(erp).(condName).(tgtGrp).data = aaaa(:, 2);
                end

            end

            Data.(erp).(condName).(tgtGrp).source_microstate = srcIdx;
            Data.(erp).(condName).(tgtGrp).target_microstate = tgtIdx;
            Data.(erp).(condName).(tgtGrp).source_dur = nanmean(Dur_src.(erp).(condName).(tgtGrp).data, 2);
            Data.(erp).(condName).(tgtGrp).target_dur = nanmean(Dur_trg.(erp).(condName).(tgtGrp).data, 2);
    ...
    for c = 1: numel(Conds)
        ...
        for g = 1: numel(TargetList)
            ...
            for pop = 1: length(sSig)
                source_dur(pop) = Data.(erp).(condName).(tgtGrp).source_dur(Data.(erp).(condName).(tgtGrp).source_microstate == sSig(pop) & Data.(erp).(condName).(tgtGrp).target_microstate == tSig(pop));
                target_dur(pop) = Data.(erp).(condName).(tgtGrp).target_dur(Data.(erp).(condName).(tgtGrp).source_microstate == sSig(pop) & Data.(erp).(condName).(tgtGrp).target_microstate == tSig(pop));
            end
